...
resource "aws_security_group_rule" "eks_node_bastion" {
  ...
  protocol                 = "tcp"
  security_group_id        = aws_security_group.eks_node_sg.id
  source_security_group_id = aws_security_group.bastion_eks_sg[0].id
  to_port                  = 22
  type                     = "ingress"
}
...
resource "aws_launch_template" "eks_worker_lt_mixed" {
  ...
  disable_api_termination = lookup(
    var.worker_launch_template_lst[count.index],
    "disable_api_termination",
    local.worker_lt_defaults["disable_api_termination"],
  )

  iam_instance_profile {
    name = element(
      aws_iam_instance_profile.eks_node_profile.*.name,
      count.index,
    )
  }
  ...
  placement {
    tenancy = lookup(
      var.worker_launch_template_lst[count.index],
      "placement_tenancy",
      local.worker_lt_defaults["placement_tenancy"],
    )
  }
  ...
  user_data = base64encode(
    element(data.template_file.eks_node_userdata.*.rendered, count.index),
  )
  instance_initiated_shutdown_behavior = lookup(
    var.worker_launch_template_lst[count.index],
    "instance_shutdown_behavior",
    local.worker_lt_defaults["instance_shutdown_behavior"],
  ) # defaults to stop
  ...
  ebs_optimized = lookup(
    ...
    lookup(
      ...
      lookup(
        var.worker_launch_template_lst[count.index],
        "instance_type_pool1",
        local.worker_lt_defaults["instance_type_pool1"],
      ),
      false,
    ),
  )
  ...
  block_device_mappings {
    device_name = lookup(
      ...
      local.worker_lt_defaults["root_block_device_name"],
    )
    ...
    ebs {
      ...
      encrypted = lookup(
        var.worker_launch_template_lst[count.index],
        "root_encrypted",
        local.worker_lt_defaults["root_encrypted"],
      )
      kms_key_id = lookup(
        var.worker_launch_template_lst[count.index],
        "root_kms_key_id",
        local.worker_lt_defaults["root_kms_key_id"],
      )
...
resource "aws_autoscaling_group" "eks_mixed_instances_asg" {
  ...
  name = "${element(aws_launch_template.eks_worker_lt_mixed.*.name, count.index)}-${lookup(
    var.worker_launch_template_lst[count.index],
    "name",
    count.index,
  )}-asg"
  ...
  mixed_instances_policy {
    instances_distribution {
      on_demand_allocation_strategy = lookup(
        ...
        local.worker_lt_defaults["on_demand_allocation_strategy"],
      )
      on_demand_base_capacity = lookup(
        var.worker_launch_template_lst[count.index],
        "on_demand_base_capacity",
        local.worker_lt_defaults["on_demand_base_capacity"],
      )
      on_demand_percentage_above_base_capacity = lookup(
        var.worker_launch_template_lst[count.index],
        "on_demand_percentage_above_base_capacity",
        local.worker_lt_defaults["on_demand_percentage_above_base_capacity"],
      )
      spot_allocation_strategy = lookup(
        var.worker_launch_template_lst[count.index],
        "spot_allocation_strategy",
        local.worker_lt_defaults["spot_allocation_strategy"],
      )
      spot_instance_pools = lookup(
        var.worker_launch_template_lst[count.index],
        "spot_instance_pools",
        local.worker_lt_defaults["spot_instance_pools"],
      )
    ...
    launch_template {
      launch_template_specification {
        ...
        version = lookup(
          ...
          element(
            aws_launch_template.eks_worker_lt_mixed.*.latest_version,
            count.index,
          ),
        )
      ...
      override {
        instance_type = lookup(
          var.worker_launch_template_lst[count.index],
          "instance_type_pool2",
          local.worker_lt_defaults["instance_type_pool2"],
        )
      }
      ...
      override {
        instance_type = lookup(
          var.worker_launch_template_lst[count.index],
          "instance_type_pool3",
          local.worker_lt_defaults["instance_type_pool3"],
        )
      }
    }
  }
  ...
  depends_on = [aws_launch_template.eks_worker_lt_mixed]
